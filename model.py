...
# Place create_model after unique_labels so len(unique_labels) is defined

def create_model(input_shape=(300, 300, 3), output_shape=None):
  if output_shape is None:
    output_shape = len(unique_labels)
  feature_extractor_url = "https://tfhub.dev/google/efficientnet/b3/feature-vector/1"
  inputs = tf.keras.Input(shape=input_shape)
  x = hub.KerasLayer(feature_extractor_url, trainable=False)(inputs)
...
# Load the model globally so it is available for prediction
